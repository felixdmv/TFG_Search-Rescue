import test.webApp.settingsTest as settingsTest
import utils.entradaSalida as es
import utils.graficosImagenes as gi
import test.webApp.prediccionTest as pred
import test.webApp.widgetsTest as wd
from transitions import Machine
import test.webApp.gpsTest as gpsTest
...
def inicializaEstadosYFSM():
    ...
    parametros = cargaParametrosConfiguracion(settingsTest.PATH_PARAMETROS)
    if parametros == None:
    ...
    try:
        ...
        st.rerun()
    try:
        st.session_state['nombresColeccionesYPath'] = cargaColecciones(settingsTest.PATH_COLECCIONESIMAGENES)
    except Exception as e:
...
def cargaColecciones(path):
    ...
    for path in coleccionesPath:
        parametros = cargaParametrosConfiguracion(path.joinpath(settingsTest.DESCRIPCION_COLECCION))
    ...
    for i, imagen in enumerate(st.session_state.listaImagenes):
        ...
        with st.spinner(f'Procesando imagen {i+1} de {numImagenes}'):
            try:
                ...
                if hayHumano:
                    latitud, longitud = gpsTest.extraeLatitudLongitud(imagen)
...
def main():
    ...
    st.logo(str(settingsTest.PATH_LOGO), link=settingsTest.URL_LOGO)
