...
def sliderSolapamientos(size, solapamiento):
    ...
    col1, col2 = st.columns(2)
    with col1:
        solape_x = st.slider('Solapamiento horizontal', solap_x, width//2, solap_x, 1)
    with col2:
        solape_y = st.slider('Solapamiento vertical', solap_y, height//2, solap_y, 1)
...
def seleccionAjustes(parametros):
    with st.expander("Selección de ajustes", expanded=True):
        ...
        with st.container(border=True):
            solape_x, solape_y = sliderSolapamientos(parametros['dimensiones'][:2], parametros['solapamiento'])
        with st.container(border=True):
            muestraEtiquetas()
    return umbral, solape_x, solape_y
#######################################################################################################################