...
def copiar_imagenes(path_imagenes, df_imagenes, path_destino):
    for indice, fila in df_imagenes.iterrows():
        ...
        ruta_imagen = os.path.join(path_imagenes, dataset, dataset, fila['Nombre del archivo'])
...
# %%
def calcular_rendimiento_test(modelo, test_dir, analisis, params_red, thresholds=[0.5]):
    ...
    for thres in thresholds:
        resultado = metricas_binclass(y_test_true, y_test_prob, thres)
...
visualkeras.layered_view(modelo).show() # display using your system viewer
visualkeras.layered_view(modelo, to_file='modelovisual.png') # write to disk
visualkeras.layered_view(modelo, to_file='modelovisual.png').show() # write and show
...
resultados_test = calcular_rendimiento_test(modelo, directorios['test'], analisis, params_red, thresholds=analisis["thresholds"])
